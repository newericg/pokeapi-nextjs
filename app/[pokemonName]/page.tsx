...
export default async function PokemonPage({
  ...
  return (
    <>
      <Card
        className={`max-w-xl grid p-2 justify-items-center pokemon-type type-${primaryTypeColour}`}
      >
        <div
          className={`text-4xl text-bold grid justify-items-center w-full px-5`}
        >
            ...
            <div className="flex justify-items-center w-full py-2">
              {pokemonObject.types.map((typeObject: any, index: number) => {
                return (
                  ...
                  >
                    <Button
                      className={`mx-2 rounded-full p-2 px-6 pokemon-type type-${typeObject.type.name}`}
                    >
                      {typeObject.type.name.charAt(0).toUpperCase() +
                        typeObject.type.name.slice(1)}
                    </Button>
                  </div>
                );
              })}
            ...
            {pokemonObject.stats.map((statObject: any) => {
              ...
              return (
                <div className="flex items-stretch my-4" key={statName}>
                  ...
                  <Progress
                    className={`w-2/4 m-auto bg-gray-300 `}
                    value={statValue}
                    indicatorColor={`pokemon-type type-${primaryTypeColour}`}
                  />
                </div>
              );
            })}
